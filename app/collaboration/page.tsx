...
function CollaborationHeader({ isObserver }: { isObserver: boolean }) {
  const { connectionStatus, connectedUsers, localUser, error, retry } =
    useCollaboration();
  ...
  return (
    <div className="flex flex-col shrink-0">
      {error && (
        <div
          className="flex items-center justify-between px-2 py-2 border-b text-sm"
          role="alert"
          style={{
      ...
      {!error && connectionStatus === "offline" && (
        <div
          ...
          role="status"
          style={{
            backgroundColor: theme.colors.bgChecked,
